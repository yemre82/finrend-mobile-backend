...
def accept_a_finrend(request):
    ...
    if request.user != finrend_obj.finrendered_user:
        return response_400("You can't accept this finrend")
    if finrend_obj.is_finrend_accepted!=None:
        return response_400("This finrend is already accepted or rejected")
    finrend_obj.is_finrend_accepted = True
    finrend_obj.save()
    try:
        user_obj=CustomUser.objects.get(id=finrend_obj.finrender_user.id)
    except ObjectDoesNotExist as e:
        return response_400("There is no such user")
    user_obj.finrenderer_count+=1
    user_obj.save()
    request.user.finrenderer_count+=1
    request.user.save()
